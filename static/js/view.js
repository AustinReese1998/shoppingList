...
class View extends Subject{
	...
	replaceTable(){
		...
		for (let i = 0; i < listArray.length; i++){
			...
			boxCol.classList.add("text-center")
	...
	replacePurchasedTable(){
	cbId = 0
	let listArray = purchasedList.getShoppingList()
	let myTable = document.querySelector("#nextItem")
	myTable.innerHTML = ""
	for (let i = 0; i < listArray.length; i++){
		cbId = cbId + 1
		...
		box.classList.add("form-control")
		box.setAttribute("id", "box" + cbId)
		box.onchange = purchasedBoxClick
		let boxCol = document.createElement("td")
		boxCol.innerHTML = box.value
		boxCol.classList.add("text-center")

		let timeCol = document.createElement("td")
		timeCol.innerHTML = listArray[i].getBought()
